...
## Fourth hosted checkpoint and PR API compatibility

Candidate `9118ccea032a743d1b1407391416516ec2ee9037`, tree `4e5eb5b2c85591f0286d8daabe59dcd4afabc125`, passed 1,141 local tests with no skips. Hosted push `34790693568` and PR `34790696238` passed all 18 jobs, including Windows (77 tests each), macOS (108 each), Python 3.10–3.12 (1,140 each, with packaging checked separately), X11 (12 each), pinned CPU inference, and wheel/standalone/AppImage builds and smokes. These remain successful application receipts.

The controller then stopped before qualification or merge because the shared backend selected API version `2026-03-10`, whose PR response intentionally removes `merge_commit_sha`. The source-only adapter now pins individual PR reads to the supported `2022-11-28` response contract and requires the returned PR identity and merge field. It does not invent a merge SHA, substitute a branch head, relax tree/parent checks or retry a mutation. A null merge field while GitHub calculates mergeability still cannot pass qualification. All other API calls, recovery journals, binary verifier code and release gates remain unchanged. The repaired candidate must pass its own exact source and hosted acceptance again.

See GitHub's [documented breaking change](https://docs.github.com/en/rest/about-the-rest-api/breaking-changes#version-2026-03-10) and [supported API versions](https://docs.github.com/en/rest/about-the-rest-api/api-versions). At this checkpoint, GitHub lists `2022-11-28` support through March 10, 2028. Requalify the adapter before that contract is retired; an unsupported version or malformed response must stop the route. No source completion or native release success is asserted by this checkpoint.
