...
def eligible(record: dict) -> bool:
    """Evaluates whether an operation record possesses valid authority for automated rollback.

    Args:
        record: The journal operation record dictionary containing state and manifest metadata.

    Returns:
        bool: True if the operation is unresolved and contains a version 2 manifest, False otherwise.
    """
...
def explanation(record: dict) -> str:
    """Generates a human-readable explanation and recorded file path manifest for a recovery record.

    Args:
        record: The journal operation record dictionary.

    Returns:
        str: A formatted multi-line summary of the operation, review reasons, and file manifests.
    """
...
class RecoveryDialog(QDialog):
    def __init__(self, viewer):
        ...
        self.status.setAccessibleName("Recovery status summary")
        self.status.setAccessibleDescription("Provides current status and feedback on recovery operations.")
        ...
        self.rollback.setAccessibleDescription("Requests an automated rollback for the selected interrupted operation.")
    ...
    def keyPressEvent(self, event):
        """Dismisses the recovery dialog upon Escape key press."""
