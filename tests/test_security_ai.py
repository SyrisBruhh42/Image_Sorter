...
from imagesorter.ai_tagger import ModelDownloader, write_metadata, AITagger
...
def test_model_downloader_sha256_mismatch(qtbot, tmp_path):
    ...
    assert "Cryptographic" in msg or "Failure" in msg or "Mismatch" in msg or "mismatch" in msg
    # Verify temporary file was unlinked and model file was never created
    assert not (tmp_path / "mobilenetv2.onnx").exists()
    assert len(list(tmp_path.glob("dl_model_*.tmp"))) == 0
